...
import {
  ...
  Building2, MapPin, Layers, Activity, Bell, HandCoins, HeartHandshake,
  ChevronRight, Inbox, Hash, BadgeCheck,
} from "lucide-react";
    ...
    (async () => {
      ...
      try {
        const [s, apps, notifs] = await Promise.all([
          api.get<StatsResponse>(`/api/applications/stats/overview?role=wakil_ngo`),
    ...
    <div className="space-y-6">
      ...
      >
        <div className="space-y-1.5">
          <p className="text-xs text-muted-foreground font-medium">Papan Pemuka NGO · Trek 2 Geran PEKB</p>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-gradient-primary">
            Selamat datang, {user?.namaPenuh?.split(" ")[0] || "Wakil"}
          </h1>
          {ngo && (
            <div className="flex flex-wrap items-center gap-2 pt-1">
              <Badge variant="outline" className="bg-accent/5 border-accent/30 text-accent-foreground">
                <Building2 className="w-3 h-3 mr-1" />
                {ngo.namaNgo}
              </Badge>
              <Badge variant="secondary" className="font-mono">{ngo.noAkreditasiPekb}</Badge>
            </div>
          )}
        </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <StatCard
          ...
          color="amber"
          loading={loading}
          trend={`${stats?.byStatus?.semakan_pbt_ngo ?? 0} semakan NGO`}
        />
        <StatCard
          icon={CheckCircle2}
          label="Diluluskan"
          ...
          color="emerald"
          loading={loading}
          trend={`${stats?.bilTrek2Diluluskan ?? 0} Trek 2`}
        />
        <StatCard
          icon={XCircle}
          label="Ditolak"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        {/* Side column: NGO profile + notifications */}
          ...
          <Card className="glass-card rounded-2xl border-border/40">
            ...
            <CardContent className="space-y-3 text-sm">
              {loading ? (
                ...
                <>
                  <ProfileRow icon={Building2} label="Nama NGO" value={ngo.namaNgo} />
                  <ProfileRow icon={Hash} label="No. ROS" value={ngo.noPendaftaranRos} mono />
                  <ProfileRow icon={BadgeCheck} label="No. Akreditasi PEKB" value={ngo.noAkreditasiPekb} mono />
                  <ProfileRow icon={MapPin} label="Negeri Operasi" value={ngo.negeriOperasi} />
                  <ProfileRow icon={MapPin} label="Daerah Operasi" value={ngo.daerahOperasi} />
                  <Separator />
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground">Status Akreditasi</span>
                    <Badge variant="outline" className="capitalize bg-emerald-50 text-emerald-700 border-emerald-200">
                      {prettyLabel(ngo.statusAkreditasi)}
                    </Badge>
                  </div>
                </>
              ) : (
                <p className="text-xs text-muted-foreground">Profil NGO tidak tersedia.</p>
              )}
            </CardContent>
          </Card>
...
function TrekInfoSection() {
  const items = [
    {
      icon: HandCoins,
      title: "Geran Ekonomi / Mikro-usahawan",
      desc: "Geran pembiayaan mikro-usahawan B40 untuk memulakan atau mengembangkan perniagaan kecil.",
      accent: "bg-teal-100 text-teal-700 border-teal-200",
    },
    {
      icon: HeartHandshake,
      title: "Bantuan Sara Hidup",
      desc: "Bantuan kewangan sekali gus untuk keluarga Miskin Tegar dan B40 yang memerlukan sokongan segera.",
    ...
    <Card className="glass-card rounded-2xl border-border/40">
      <CardHeader>
        ...
        <CardDescription className="text-xs">
          Dua kategori bantuan di bawah Trek 2 yang ditadbir oleh NGO akreditasi PEKB.
        </CardDescription>
      </CardHeader>
